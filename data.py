...
nas360_cfg = dict(
    fsd_root = "/localdisk/home/lericsso/code/einspace/data/fsd50k/nasbench360/",
    darcy_root = "/localdisk/home/lericsso/code/einspace/data/darcyflow/",
    psicov_root = "/localdisk/home/lericsso/code/einspace/data/psicov",
    cosmic_root = "/localdisk/home/lericsso/code/einspace/data/cosmic/new",
    ecg_root = "/localdisk/home/lericsso/code/einspace/data/ecg",
    satellite_root = "/localdisk/home/lericsso/code/einspace/data/satellite",
    deepsea_root = "/localdisk/home/lericsso/code/einspace/data/deepsea"
)
...
class SwitchDataset(Dataset):
    def __init__(self, csv_path):
        df = pd.read_csv(csv_path, header=None).astype("float32")
        self.x = torch.tensor(df.iloc[:, :15 ].values)   # (N, 15)
        self.y = torch.tensor(df.iloc[:, 15:].values)    # (N, 6)
...
def get_data_loaders(
    ...
    elif dataset == "switch15to6":
        trainset = SwitchDataset(Path(root) / "Switch15to6" / "train.csv")
        valset = SwitchDataset(Path(root) / "Switch15to6" / "val.csv")
        testset = SwitchDataset(Path(root) / "Switch15to6" / "test.csv")
